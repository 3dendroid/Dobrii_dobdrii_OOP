...
class TicTacToe:
    ...
    def __update_win_status(self):
        ...
        if all(self.pole[i][i].value == self.HUMAN_X for i in range(self._size)) or all(
                self.pole[i][-1 - i].value == self.HUMAN_X for i in range(self._size)):
            self._win = 1
            return

        if all(self.pole[i][i].value == self.COMPUTER_O for i in range(self._size)) or all(
                self.pole[i][-1 - i].value == self.COMPUTER_O for i in range(self._size)):
            self._win = 2
            return
    ...
    def human_go(self):
        ...
        while True:
            i, j = map(int, input("Ввведите координаты клетки: ").split())
    ...
    def __bool__(self):
        return self.is_human_win == 0 and self._win not in (1, 2, 3)
